...
class myDB extends SQLite3
{
    ...
    public static function obtenerHabitacionesDisponiblesSegunFechaPiso($fecha, $piso)
    {
        $limiteRegistrosRevisados = 150;
        //aqui ahi un limite de las habitaciones analizadas, las ultimas 150
        $auxFecha = $fecha;
        $db = self::getInstance();
        $sql = "SELECT r.id as id, h.nombre,  r.activo, c.nombre as cliente, r.fechaSalida
        ...
        INNER JOIN  habitaciones  as h  on r.idHabitacion = h.id
        INNER JOIN  clientes  as c on c.id = r.idCliente
        INNER JOIN posicionHabitaciones as p on h.id  =p.id_habitacion
        WHERE (:fecha >= r.fechaEntrada AND :fecha < r.fechaSalida) AND  p.piso = $piso
        ORDER BY r.id DESC
        limit $limiteRegistrosRevisados
         ";
        ...
        INNER JOIN  mantenimiento  as m on m.id_habitacion = h.id
         INNER JOIN posicionHabitaciones as p on h.id  =p.id_habitacion
        WHERE (:fecha2 >= m.fecha_inicio AND :fecha2 <= m.fecha_final)  AND  p.piso = $piso
        ORDER BY m.id DESC
        limit $limiteRegistrosRevisados
         ";
    ...
    public static function obtenerTasaBcv($fecha = "actual")
    {
        ...
        if (!$resultado) {
            $fechaAuxActual = new DateTime();
            $bs  = myDB::obtenerUltimaTasaDisponible($fechaAuxActual);
        } else {
            $bs =  $resultado;
        }
    ...
    public static function obtenerUltimaTasaDisponible($fechaActual)
    {
        $db = self::getInstance();


        // $fechaObjetivo = new DateTime($fechaActual);
        $fechaObjetivo = $fechaActual;
    ...
    public static function ingresarRentaHabitacion($data)
    {
        ...
        // $idPago = $data["idPago"];
        $activo = $data["activo"];
        $incidencia = $data["incidencia"] == "" ? NULL : $data["incidencia"];
        // $resultado="hola";


        $sql =
            'INSERT INTO "rentaHabitacion" ("idCliente", "idHabitacion", "fechaEntrada", 
        "fechaSalida", "activo", "nota")
        VALUES (:idCliente, :habitacion, :fechaEntrada, :fechaSalida,  :activo, :incidencia)';
        ...
        $stmt->bindValue(':incidencia', $incidencia, SQLITE3_TEXT);
    ...
    public static function actualizarIncidenciaRenta($id, $incidencia)
    {
        $db = self::getInstance();
        ...
        $sql = 'update rentaHabitacion 
                    ...
                    WHERE id= :id';
        $consulta = $db->prepare($sql);
        $consulta->bindValue(":nota", $incidencia, SQLITE3_TEXT);
    ...
    public static function  obtenerTotalReservaciones()
    {
        $db = self::getInstance();

        $sql = "SELECT COUNT(DISTINCT r.id) as total FROM reservacion as r
                ";
    ...
    public static function obtenerListaReservaciones($salto = 0, $limite = 9999999, $campo = false, $texto = false)
    {
        // $limite = 25;
        $db = self::getInstance();
        ...
        $stmt->bindValue(":salto", $salto, SQLITE3_INTEGER);
        ...
        while ($fila = $resultado->fetchArray(SQLITE3_ASSOC)) {
            ...
            if ($campo != "") {
                //condicional por que el campo es verdadero o falso
                if ($campo == "estadoPago") {
                    if (strtoupper($texto) == "SI" || strtoupper($texto) == "PAGADO") $texto = true;
                    else if (strtoupper($texto) == "NO" || strtoupper($texto) == "PENDIENTE") $texto = false;

                    if ($fila[$campo] != $texto) $van = false;
                } elseif ($campo == "estado") {

                    if (strtoupper($texto) == "CADUCA") $texto = 0;
                    elseif (strtoupper($texto) == "ACTIVA") $texto = 1;
                    elseif (strtoupper($texto) == "COMPLETADA") $texto = 2;
                    if ($fila[$campo] != $texto) $van = false;
                } else if (!str_contains(strtoupper($fila[$campo]), strtoupper($texto))) $van = false;
            }
            if ($van) {
                $listaReservaciones[] = $fila;
            }
    ...
    //  public static function obtenerListaIdReservaciones()
    // {
    //     // $limite = 25;
    //     $db = self::getInstance();
    ...
    //     }
    //     $listaIdReservaciones = [];



    //     return $listaIdReservaciones;
    // }
        ...
        while ($fila = $resultado->fetchArray(SQLITE3_ASSOC)) {
            ...
            if (
                ...
                if ($fila["base"] == 0) {
                    ...
                    $tasaAuxiliar = myDB::obtenerUltimaTasaDisponible($fechaPago);
    ...
    public static function obtenerDatosDashboar()
    {
        ...
        $fechaActual = new DateTime();
        $tasa = myDB::obtenerUltimaTasaDisponible($fechaActual);
        ...
        $data = [
            "tasa" => $tasa["tasa"],
            "tasaFecha" => tools::fechaF_dmy($tasa["fecha"]),
    ...
    public static function obtenerListaIngresosTotales($fecha, $campo = "", $texto = "")
    {
        $db = self::getInstance();

        $sql = "SELECT r.id, h.nombre, c.nombre as cliente, r.fechaEntrada, r.fechaSalida, r.nota FROM rentaHabitacion as r
                INNER JOIN habitaciones as h on r.idHabitacion = h.id
        ...
        $fechaObjetivo = ($fecha != false) ? new DateTime($fecha) : false;
        ...
        while ($fila = $resultado->fetchArray(SQLITE3_ASSOC)) {
            ...
            if ($fechaObjetivo) {
                ...
                if ($fechaObjetivo->format("Y-m") !== $fechaAux->format("Y-m")) {
                    $van = false;
                }
            }
            if ($campo != "") {
    ...
    public static function obtenerListaSalidasTotales($fecha)
    {
        $db = self::getInstance();

        $sql = "SELECT r.id, h.nombre, c.nombre as cliente, r.fechaEntrada, r.fechaSalida 
                FROM rentaHabitacion as r
                INNER JOIN habitaciones as h on r.idHabitacion = h.id
                INNER JOIN clientes as c on r.idCliente = c.id
                -- Hacemos el cruce opcional con reservacion
                LEFT JOIN reservacion as res on res.idRentaHabitacion = r.id
                WHERE 
                    res.idRentaHabitacion IS NULL  -- Caso A: No tienen una reservación relacionada
                    OR res.estado = 2             -- Caso B: Sí tienen reservación Y su estado es 2
                ORDER BY r.id DESC";
        ...
        // Creamos el objeto DateTime con la fecha recibida
        $fechaObjetivo = ($fecha != false) ? new DateTime($fecha) : false;
        ...
        while ($fila = $resultado->fetchArray(SQLITE3_ASSOC)) {
            ...
            // 1. FILTRADO POR FECHA DE SALIDA (EXACTAMENTE EL MISMO DÍA)
            if ($fechaObjetivo) {
                // Si no hay fecha de salida registrada en la fila, no puede haber salido ese día
                if (empty($fila["fechaSalida"])) {
                    $van = false;
                } else {
                    $fechaSalidaAux = new DateTime($fila["fechaSalida"]);

                    // Comparamos Año-Mes-Día (Y-m-d) en lugar de solo Año-Mes (Y-m)
                    if ($fechaObjetivo->format("Y-m-d") !== $fechaSalidaAux->format("Y-m-d")) {
                        $van = false;
                    }
                }
            }
            ...
            // Si pasó todos los filtros, se agrega al array
            if ($van) {
                $salidasHabitaciones[] = $fila;
            }
        }
        return $salidasHabitaciones;
    }
    public static function obtenerListaEntradasTotales($fecha)
    {
        $db = self::getInstance();

        // 1. Base de la consulta con el LEFT JOIN de reservación
        $sql = "SELECT r.id, h.nombre, c.nombre as cliente, r.fechaEntrada, r.fechaSalida 
            FROM rentaHabitacion as r
            INNER JOIN habitaciones as h on r.idHabitacion = h.id
            INNER JOIN clientes as c on r.idCliente = c.id
            LEFT JOIN reservacion as res on res.idRentaHabitacion = r.id
            WHERE (res.idRentaHabitacion IS NULL OR res.estado = 2)";
        // Nota: Si para ingresos el estado de la reservación es otro (ej. 1), cámbialo aquí.

        // 2. Filtramos DIRECTAMENTE por el día de fechaEntrada usando date() de SQLite
        if ($fecha != false) {
            $sql .= " AND date(r.fechaEntrada) = :fechaObjetivo";
        }
        ...
        // Enlazamos el parámetro de la fecha de forma segura
        if ($fecha != false) {
            $fechaFormateada = (new DateTime($fecha))->format("Y-m-d");
            $stm->bindValue(':fechaObjetivo', $fechaFormateada, SQLITE3_TEXT);
        }

        $resultado = $stm->execute();
        $ingresosHabitaciones = [];

        // El bucle ahora es limpísimo porque SQLite ya hizo todo el filtro
        while ($fila = $resultado->fetchArray(SQLITE3_ASSOC)) {
            $fila["estadoPago"] = myDB::comprobarRentaPagado($fila["id"]);

            // Ya no necesitas ifs complejos de DateTime aquí adentro, 
            // SQLite solo te devolvió las que entraron ese día exacto.
            $ingresosHabitaciones[] = $fila;
        }
    ...
    public static function obtenerNumeroClientes()
    {
        ...
        $sql = "SELECT COUNT(DISTINCT c.id) as total FROM clientes as c
    ...
    public static function obtenerListaClientes($desplazamiento = 0, $numeroRegistros = 9999999, $campo = false, $texto = "")
    {
        $db = self::getInstance();
        ...
        $sql = "SELECT 
                ...
                c.ciudad, 
                c.empresa, 
                MAX(r.fechaEntrada) AS ultimaFechaEntrada
            FROM clientes c
            INNER JOIN rentaHabitacion r ON c.id = r.idCliente
            GROUP BY 
                c.id, 
                c.nombre, 
                c.ci, 
                ...
                c.empresa
            ORDER BY c.id DESC
            LIMIT :numeroRegistros OFFSET :desplazamiento;";



        $stmt = $db->prepare($sql);
        $stmt->bindValue(':desplazamiento', $desplazamiento, SQLITE3_INTEGER);
        ...
        while ($fila = $resultado->fetchArray(SQLITE3_ASSOC)) {
            ...
            if ($texto != "") {
                if (str_contains($fila[$campo], $texto)) {
                    $listaClientes[] = $fila;
                }
            } else {
                $listaClientes[] = $fila;
    ...
    public static function obtenerNumeroPagos()
    {
        $db = self::getInstance();

        $sql = "SELECT COUNT(DISTINCT p.id) as total FROM pagos as p
    ...
    public static function obtenerListaDatosPagos($salto, $limite, $campo = "false", $texto = "")
    {
        $db = self::getInstance();
        ...
        while ($fila = $resultado->fetchArray(SQLITE3_ASSOC)) {
            ...
            $tasa = myDB::obtenerUltimaTasaDisponible($fecha);
            ...
            if ($texto != "") {
                ...
                if (str_contains(strtoupper($datos[$campo]), strtoupper($texto))) {
                    $listaDatosPagos[] = $datos;
                }
            } else {
                $listaDatosPagos[] = $datos;
            }
    ...
    public static function obtenerTotalReparaciones()
    {
        $db = self::getInstance();
    ...
    public static function obtenerListaMantenimiento($salto, $limite)
    {
        $db = self::getInstance();
    ...
    public static function obtenerNumeroTasas()
    {
        $db = self::getInstance();

        $sql = "SELECT count(DISTINCT tc.id) as total FROM tasas_cambio as tc
    ...
    public static function obtenerListaTasas($salto, $limite)
    {
        $db = self::getInstance();
        ...
        $stmt = $db->prepare($sql);
        $stmt->bindparam(":limite", $limite, SQLITE3_INTEGER);
        $stmt->bindparam(":salto", $salto, SQLITE3_INTEGER);
    ...
    public static function obtenerPisos()
    {
        $db = self::getInstance();
    ...
    public static function obtenerPosicionesHabitacionesTodosPisos()
    {
        $db = self::getInstance();

        $pisos = myDB::obtenerPisos();

        $listaPosicionesTotal = [];
        ...
        foreach ($pisos as $index => $piso) {
            ...
            $listaPosicionesTotal[] = ["nombre" => $piso["nombre"], "id" => $piso["id"], "datosHabitaciones" => $posiciones];
        }




        return $listaPosicionesTotal;
    }
    ...
    public static function registrarNuevoPiso($nombre, $descripcion)
    {

        $db = self::getInstance();
        $sql = ' INSERT INTO "pisos" ("nombre", "descripcion") 
      ...
      public static function actualizarPiso($id,$nombre, $descripcion)
    {
        ...
        $sql = 'UPDATE "pisos" SET nombre= :nombre , descripcion= :descripcion
        WHERE id = :id
        ;';
    ...
    public static function reiniciarBaseDeDatos()
    {
        // 1. ¡SEGURIDAD ANTE TODO! 
        // Valida estrictamente que sea el administrador principal.

        // 2. Rutas de los archivos

        $ruta_sql = "../../model/dbsystemhotel_inicial.sql"; // Tu archivo con los scripts


        // 3. Validar que el archivo SQL exista antes de hacer nada
        if (!file_exists($ruta_sql)) {
            die("Error: El archivo SQL de fábrica no se encuentra");
        }

        // 4. Leer todo el contenido del archivo SQL
        $script_sql = file_get_contents($ruta_sql);
        if ($script_sql === false) {
            die("Error al leer el archivo SQL.");
        }

        $db = self::getInstance();
        // 5. Conectarse a la base de datos SQLite

        // Habilitar las excepciones para capturar cualquier error de sintaxis SQL
        $db->enableExceptions(true);

        // 6. Ejecutar TODO el script de golpe
        // SQLite3::exec permite múltiples sentencias separadas por punto y coma (;)
        $db->exec($script_sql);
    }
